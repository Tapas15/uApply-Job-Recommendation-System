...
class ResourceManager:
    ...
    @staticmethod
    def load_embeddings():
        with open(os.path.join(PROJECT_ROOT, 'notebooks', 'embeddings_mpnet.pkl'), 'rb') as f:
            ResourceManager.embeddings_dataset = pickle.load(f)
        with open(os.path.join(PROJECT_ROOT, 'notebooks', 'students_position_embeddings.pkl'), 'rb') as f:
            ResourceManager.students_skills_embedding = pickle.load(f)
        with open(os.path.join(PROJECT_ROOT, 'notebooks', 'knn_model.pkl'), 'rb') as f:
            ResourceManager.knn = pickle.load(f)
        ResourceManager.job_posting = pd.read_csv(os.path.join(PROJECT_ROOT, 'notebooks', 'job_posting_data.csv'))
        ResourceManager.students = pd.read_csv(os.path.join(PROJECT_ROOT, 'notebooks', 'students5.csv'))
